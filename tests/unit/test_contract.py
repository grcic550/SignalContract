...
from signalcontract.errors import ValidationError, EventParseError
...
def test_expect_not_dict_raises_validation_error():
    ...
    with pytest.raises(ValidationError, match=r"Contract expect field must be a dictionary\."):
        Contract.from_dict(contract)
...
def test_expect_with_non_string_keys_raises_validation_error():
    ...
    with pytest.raises(ValidationError, match=r"Contract expect field must be a dictionary of string keys and string values\."):
        Contract.from_dict(contract)
...
def test_expect_misspelled_key_raises_validation_error():
    ...
    contract = {
        ...
        "scenario": "denied_admin_db_deletion",
        "expect":{
            "outocme": "authorization.denied"
        }
    }

    with pytest.raises(
        ValidationError, 
        match=r"Contract expect field contains invalid key 'outocme', did you mean 'outcome'?"
    ):
        Contract.from_dict(contract)
...
def test_contract_version_out_of_range_raises_validation_error():
    ...
    with pytest.raises(
        ValidationError,
        match=r"Contract version must be an integer and equal to 1 because only version 1 is supported\."
    ):
        Contract.from_dict(contract)
